...
static void scroll_event_cb(lv_event_t*);

static void wifi_network_button_clicked(lv_event_t* e)
{
    wifi_scan_result* clicked_network = (wifi_scan_result*) lv_event_get_user_data(e);
    connect_to_wifi_network(clicked_network, "password");

    // Show which network we're connecting to and an input for the password.
    lv_obj_t* network_list = lv_obj_get_child(lv_screen_active(), -1);
    lv_obj_delete(network_list);

    // Create a widget for the password entry.
    lv_obj_t* password_entry = lv_obj_create(lv_screen_active());
    lv_obj_set_style_text_color(password_entry, lv_color_white(), LV_PART_MAIN);
    lv_obj_set_style_border_side(password_entry, LV_BORDER_SIDE_NONE, 0);
    lv_obj_set_style_bg_color(password_entry, lv_color_black(), LV_PART_MAIN);
    lv_obj_set_flex_flow(password_entry, LV_FLEX_FLOW_COLUMN);
    lv_obj_align(password_entry, LV_ALIGN_CENTER, 0, -20);
    lv_obj_set_width(password_entry, 210);

    // Label for the network name.
    lv_obj_t* network_name = lv_label_create(password_entry);
    lv_label_set_text_fmt(network_name, "Password (%s)", (char*)clicked_network->ssid);

    lv_obj_t* password = lv_textarea_create(password_entry);
    lv_textarea_set_text(password, "");
    lv_textarea_set_one_line(password, true);

    static lv_obj_t* kb;
    kb = lv_keyboard_create(lv_screen_active());
    lv_obj_set_size(kb, 220, 100);
    lv_keyboard_set_textarea(kb, password);
}
...
static void scan_button_action_async(lv_timer_t*) {
    // Delete the scan button.
    lv_obj_t* scan_button = lv_obj_get_child(lv_screen_active(), -1);
    lv_obj_delete(scan_button);
    ...
    for (int i = 0; i < num_networks; i++) {
        ...
        lv_obj_add_event_cb(wifi_button, wifi_network_button_clicked, LV_EVENT_CLICKED, (void*) &networks[i]);
